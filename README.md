...
> **Note:** This project is based on [farion1231/cc-switch](https://github.com/farion1231/cc-switch/tree/main). See the original repository for upstream releases and the official website at [ccswitch.io](https://ccswitch.io).
...
- **DeepSeek Proxy** — Built-in local proxy for DeepSeek API (OpenAI Responses API ↔ DeepSeek Chat Completions API protocol conversion), powered by `ccswitch-deepseek-main` (Node.js sidecar)
...
|-----------|---------|-------|
| Node.js | ≥ 20.19 (tested: 22.12.0 x64) | Required for Vite 7 |
| pnpm | ≥ 8 | Package manager |
| Rust | ≥ 1.85.0 | MSVC toolchain (Windows) |
| Tauri CLI | 2.8+ | Managed via `@tauri-apps/cli` |

**Windows-specific requirements:**

- **Visual Studio Build Tools 2022** with "Desktop development with C++" workload (provides MSVC linker `link.exe` and Windows SDK)
- Install via winget: `winget install Microsoft.VisualStudio.2022.BuildTools --override "--wait --quiet --add Microsoft.VisualStudio.Workload.VCTools --includeRecommended"`
- If `NODE_OPTIONS` environment variable is set (e.g., `--use-system-ca`), clear it before running dev/build: `set NODE_OPTIONS=`

**DeepSeek Proxy setup (optional):**

The DeepSeek proxy module (`ccswitch-deepseek-main`) is expected as a sibling directory to the project root. Install its dependencies:
