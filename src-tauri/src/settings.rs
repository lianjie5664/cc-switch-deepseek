...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DeepSeekProxyConfig {
    #[serde(default)]
    pub enabled: bool,
    #[serde(default = "default_deepseek_port")]
    pub port: u16,
    #[serde(default)]
    pub api_key: String,
...
impl Default for DeepSeekProxyConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            port: 11435,
...
pub struct AppSettings {
    ...
    // ===== DeepSeek 本地代理 =====
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub deepseek_proxy: Option<DeepSeekProxyConfig>,
}
...
/// 获取 DeepSeek 代理配置
pub fn get_deepseek_proxy_config() -> Option<DeepSeekProxyConfig> {
    settings_store()
        .read()
        .unwrap_or_else(|e| {
            log::warn!("设置锁已毒化，使用恢复值: {e}");
