...
> **注：** 本プロジェクトは [farion1231/cc-switch](https://github.com/farion1231/cc-switch/tree/main) をベースに改変したものです。アップストリームのリリースおよび公式サイトは [ccswitch.io](https://ccswitch.io) をご参照ください。
...
- **DeepSeek プロキシ** -- 内蔵ローカル DeepSeek プロキシサービス（OpenAI Responses API ↔ DeepSeek Chat Completions API プロトコル変換）、`ccswitch-deepseek-main`（Node.js サイドカー）により駆動
...
| 依存 | バージョン | 備考 |
|------|------|------|
| Node.js | ≥ 20.19（検証済：22.12.0 x64） | Vite 7 に必要 |
| pnpm | ≥ 8 | パッケージマネージャー |
| Rust | ≥ 1.85.0 | Windows は MSVC ツールチェーンが必要 |
| Tauri CLI | 2.8+ | `@tauri-apps/cli` で管理 |

**Windows 固有の要件：**

- **Visual Studio Build Tools 2022**（「C++ によるデスクトップ開発」ワークロードを含む）— MSVC リンカー `link.exe` と Windows SDK を提供
- winget でインストール：`winget install Microsoft.VisualStudio.2022.BuildTools --override "--wait --quiet --add Microsoft.VisualStudio.Workload.VCTools --includeRecommended"`
- `NODE_OPTIONS` 環境変数（例：`--use-system-ca`）が設定されている場合、実行前にクリア：`set NODE_OPTIONS=`

**DeepSeek プロキシのセットアップ（任意）：**

DeepSeek プロキシモジュール（`ccswitch-deepseek-main`）はプロジェクトルートの兄弟ディレクトリに必要です。依存関係をインストール：
